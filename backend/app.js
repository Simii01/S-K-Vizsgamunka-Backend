...
app.post("/api/cart", authenticateToken, (req, res) => {
    ...
    pool.query(sql, [userId, product_id, quantity, quantity], (err, result) => {
        if (err) {
            console.error("SQL hiba:", err);
            return res.status(500).json({ error: "Adatbázis hiba." });
        }
...
app.get("/api/cart", authenticateToken, (req, res) => {
    ...
    const sql = `
        SELECT *,  (products.price * cart.quantity) as TOTAL_PRICE
...
app.get("/api/search/:search", (req, res) => {
    ...
    console.log(search);
    const sql = "SELECT * FROM products WHERE category LIKE ? OR brand like ? OR size LIKE ? OR color LIKE ? ";
    pool.query(sql, [search, search, search, search], (err, result) => {
        if (err) {
            console.log(err);
        ...
        if (result.length == 0) {
            return res.status(404).json({ error: "keresett elem nem talalhato" });
        }
        return res.status(200).json(result);
    });
});
